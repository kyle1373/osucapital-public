...
export default function LeaderboardTable(props: ActivityTableProps) {
                ...
                {props.users.map((user, index) => {
                  ...
                  return (
                    <tr className={`${getColor()}`} key={index}>
                      ...
                      <td
                        className={`sm:px-6 py-3 px-3 whitespace-nowrap sm:text-lg text-xs ${
                          user.coin_differential >= 0
                            ? "text-green-400"
                            : "text-red-400"
                        }`}
                      >
                        {numberWithCommas(user.coin_differential)}
                      </td>
                    </tr>
                  );
                })}
