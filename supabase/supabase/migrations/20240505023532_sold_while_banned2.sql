drop function if exists "public"."sell_all_shares"(p_user_id integer, p_stock_id integer, p_percent_return real);

drop function if exists "public"."sell_all_shares"(p_user_id integer, p_stock_id integer, p_percent_return real, stock_banned boolean);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.sell_all_shares(p_user_id integer, p_stock_id integer, p_percent_return real, p_is_stock_banned boolean)
...
BEGIN
    ...
    ORDER BY timestamp LOOP
        ...
        v_coin_total_temp_with_penalty := p_percent_return * v_coin_total_temp;
        ...
        UPDATE trades SET shares_left = 0 WHERE id = v_buy_trade.id;
    END LOOP;
    ...
    INSERT INTO trades (user_id, stock_id, type, num_shares, coins, shares_left, profit, coins_with_taxes, share_price, sold_while_banned)
    VALUES (p_user_id, p_stock_id, 'sell', v_total_shares_sold, v_coins_earned, 0, v_net_profit, v_coins_earned, 1, p_is_stock_banned);
