...
BEGIN
    ...
    SELECT 
      u.user_id,
      RANK() OVER (ORDER BY (u.coins_held + COALESCE(SUM(COALESCE(s.share_price, 0) * us.num_shares), 0)) DESC) AS global_rank
    FROM 
      users u
    ...
    FOR user_record IN SELECT * FROM users LOOP
        ...
        FOR stock_record IN SELECT us.stock_id, us.num_shares, COALESCE(s.share_price, 0) AS share_price FROM users_stocks us
