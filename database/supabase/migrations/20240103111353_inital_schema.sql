alter table "public"."friends" drop constraint "friend_pkey";

drop index if exists "public"."friend_pkey";

alter table "public"."friends" drop column "id";
...
CREATE OR REPLACE FUNCTION public.get_user_stocks(p_user_id bigint, p_stock_id bigint)
 RETURNS TABLE(stock_id bigint, share_price double precision, shares_owned double precision, share_rank bigint, share_price_change_percentage double precision, osu_id bigint, osu_name character varying, osu_picture text, osu_rank integer, osu_pp double precision, last_updated timestamp with time zone, share_price_history json)
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    FROM
        ...
        LEFT JOIN (
            SELECT
                sh.stock_id,
                json_agg(json_build_object('date', sh.date, 'price', sh.price))::json as share_price_history
            FROM
                stocks_history sh
            WHERE
                sh.stock_id = p_stock_id
            GROUP BY
                sh.stock_id
    ...
    GROUP BY
        s.stock_id, sp_history.share_price_history, latest_price.price;
END;
$function$
;

create type "public"."stock_stats" as ("stock_id" bigint, "share_price" double precision, "shares_owned" double precision, "share_rank" bigint, "share_price_change_percentage" double precision, "osu_id" bigint, "osu_name" text, "osu_picture" text, "osu_rank" bigint, "osu_std_pp" double precision, "last_updated" timestamp with time zone, "share_price_history" json);
