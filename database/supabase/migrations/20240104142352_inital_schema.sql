...
alter table "public"."stocks" drop constraint "stocks_share_price_check";

alter table "public"."trades" drop constraint "trades_coins_check";

alter table "public"."users" drop constraint "users_coins_held_check";

alter table "public"."users_stocks" drop constraint "users_stocks_num_shares_check";

drop function if exists "public"."get_user_stocks"(p_user_id bigint, p_stock_id bigint);
...
alter table "public"."stocks" alter column "osu_pp" set data type numeric using "osu_pp"::numeric;

alter table "public"."stocks" alter column "share_price" set data type numeric(10,3) using "share_price"::numeric(10,3);

alter table "public"."stocks_history" alter column "price" set data type numeric(10,3) using "price"::numeric(10,3);

alter table "public"."trades" alter column "coins" set data type numeric(10,3) using "coins"::numeric(10,3);

alter table "public"."trades" alter column "num_shares" set data type numeric(10,3) using "num_shares"::numeric(10,3);

alter table "public"."users" alter column "coins_held" set data type numeric(10,3) using "coins_held"::numeric(10,3);

alter table "public"."users_history" alter column "total_coins" set data type numeric(10,3) using "total_coins"::numeric(10,3);

alter table "public"."users_stocks" alter column "num_shares" set data type numeric(10,3) using "num_shares"::numeric(10,3);

alter table "public"."trades" add constraint "trades_num_shares_check" CHECK ((num_shares > (0)::numeric)) not valid;

alter table "public"."trades" validate constraint "trades_num_shares_check";

alter table "public"."stocks" add constraint "stocks_share_price_check" CHECK ((share_price > (0)::numeric)) not valid;

alter table "public"."stocks" validate constraint "stocks_share_price_check";

alter table "public"."trades" add constraint "trades_coins_check" CHECK ((coins > (0)::numeric)) not valid;

alter table "public"."trades" validate constraint "trades_coins_check";

alter table "public"."users" add constraint "users_coins_held_check" CHECK ((coins_held >= (0)::numeric)) not valid;

alter table "public"."users" validate constraint "users_coins_held_check";

alter table "public"."users_stocks" add constraint "users_stocks_num_shares_check" CHECK ((num_shares >= (0)::numeric)) not valid;
...
BEGIN
  ...
  SELECT
    ...
    u.osu_name,
    u.osu_picture,
    (u.coins_held + COALESCE(sum(us.num_shares * s.share_price), 0))::numeric(36,3) as total_coins,
    (u.coins_held + COALESCE(sum(us.num_shares * s.share_price), 0) - uh.total_coins)::numeric(36,3) as coin_differential_24h
  FROM
    users u
...
BEGIN
    ...
    SELECT
        s.stock_id,
        s.share_price::numeric(36,3),
        COALESCE(SUM(us.num_shares), 0.0)::numeric(36,3) as shares_owned,
        rank() OVER (ORDER BY s.share_price DESC)::bigint as share_rank,
        ((s.share_price - lag(s.share_price) OVER (ORDER BY s.last_updated)) / lag(s.share_price) OVER (ORDER BY s.last_updated) * 100)::numeric(36,3) as share_price_change_percentage,
        s.stock_id as osu_id,
        s.osu_name,
        s.osu_picture,
        s.osu_rank,
        s.osu_pp::numeric(36,3),
        s.last_updated,
